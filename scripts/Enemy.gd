...
func move(move_velocity):
	var snap = Vector2.DOWN * Constants.FLOOR_DETECT_DIST if move_velocity.y == 0.0 else Vector2.ZERO
	velocity = move_and_slide_with_snap(
	move_velocity, snap, Constants.FLOOR_NORMAL, floor_check.is_colliding(), Constants.MAX_SLIDE, Constants.FLOOR_MAX_ANGLE, false
	)
	set_look_direction()
