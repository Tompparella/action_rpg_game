extends "res://scripts/StateMachine.gd"

func _ready():
	states_map = {
		"idle": $Idle,
		"walk": $Walk,
		"jump": $Jump,
		"attack": $Attack,
		"kick": $Kick,
		"lunge": $Lunge,
		"recover": $Recover,
		"dead": $Die,
		"fall": $Fall,
	}
	
func _on_player_death():
	_change_state("dead")
	
func _on_player_fall():
	if !(current_state in [$Attack, $Kick, $Die, $Lunge]):
		_change_state("fall")
		
func _on_Player_hit(hit_damage, hit_direction, enemy_path):
	_change_state("recover")
	
func _change_state(state_name):
	if !_active:
		return
	if state_name in ["jump", "attack", "recover", "dead", "lunge"]:
		states_stack.push_front(states_map[state_name])
	if  state_name == "jump":
		$Jump.initialize($Walk.move_speed)
	._change_state(state_name)
		
# Check input that can interrupt states
func _unhandled_input(event):
	if event.is_action_pressed("attack") && (current_state != $Attack):
		if current_state in [$Recover, $Kick, $Lunge]:
			return
		_change_state("attack")
	elif event.is_action_pressed("special") && (current_state != $Lunge):
		if current_state in [$Recover, $Kick, $Attack]:
			return
		_change_state("lunge")
	current_state.handle_input(event)
